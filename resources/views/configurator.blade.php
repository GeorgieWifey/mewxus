...
<div x-data>
    ...
    <div class="fixed bottom-4 right-4 z-50 flex flex-col gap-2 w-80 pointer-events-none" aria-live="polite">
        <template x-for="t in $store.app.toasts" :key="t.id">
            <div class="pixel-panel px-3 py-2 text-[11px] font-bold border-2 pointer-events-auto"
    ...
    <div x-show="$store.app.phase === 'gate'" x-cloak class="min-h-screen checker-faint flex items-center justify-center p-4">
        <div class="pixel-panel p-6 sm:p-10 max-w-xl w-full text-center">
            ...
            <p class="mt-3 text-[11px] text-subtext1 font-bold">A tiny driver with a big heart, for your Yodall Nexus 61S.</p>

            <div x-show="$store.app.error" x-cloak class="mt-4 border-3 border-red bg-red/10 px-3 py-2 text-[11px] font-bold text-red text-left"
            ...
            <div x-show="!$store.app.webhidSupported" x-cloak
                 class="mt-5 border-3 border-yellow bg-yellow/15 px-3 py-2 text-[11px] text-left text-text">
                Your browser has no WebHID. Chrome, Edge or Opera will connect to the real board —
                or poke around in demo mode right now.
            </div>

            <p class="mt-6 text-[10px] text-subtext0 font-bold leading-relaxed">
    ...
    <div x-show="$store.app.phase === 'ready'" x-cloak class="min-h-screen flex flex-col checker-faint">
        ...
        <header class="flex items-center gap-3 px-4 py-3 bg-crust dither border-b-3 border-crust flex-wrap">
            <h1 class="font-pixel text-[11px] text-text">Mewxus</h1>
            ...
            <div class="grow"></div>
            <button type="button" class="pixel-btn bg-surface1 px-3 py-1.5 text-[10px]" @click="$store.app.disconnect()">Disconnect</button>
        </header>
        ...
        <main class="flex-1 grid gap-4 p-4 items-start lg:items-stretch grid-cols-1 lg:grid-cols-[270px_minmax(0,1fr)_380px]">
            ...
            <aside class="flex flex-col gap-4">
                <section class="pixel-panel p-4">
                    ...
                    <div class="flex flex-col items-center gap-2">
                        ...
                        <div class="text-[10px] font-bold text-subtext1 text-center"
                ...
                <section class="pixel-panel p-4">
                    ...
                    <form class="flex flex-col gap-2"
                        ...
                        <input type="text" name="name" required maxlength="80" placeholder="Preset name"
                               class="pixel-input text-[11px] w-full">
                        <input type="hidden" name="data" x-model="$store.app.presetInputValue">
                        <button type="submit" class="pixel-btn pixel-btn-blue px-3 py-1.5 text-[10px] w-full">Save current setup</button>
                    </form>
                    <div id="preset-list" class="mt-3" hx-get="/presets" hx-trigger="revealed" hx-swap="innerHTML">
                        <p class="text-[10px] text-subtext0 font-bold">loading presets...</p>
                    </div>
                </section>
            </aside>
            ...
            <section class="pixel-panel p-4 min-w-0">
                <div class="flex items-center gap-2 flex-wrap mb-3">
                    ...
                    <div class="grow"></div>
                    <button type="button" class="pixel-btn px-3 py-1.5 text-[10px]"
                ...
                <div class="mt-3 flex items-center gap-2 flex-wrap text-[10px] font-bold text-subtext1">
            ...
            <aside class="pixel-panel p-0 min-w-0">
                ...
                <div class="p-4 max-h-[calc(100vh-140px)] overflow-y-auto">
                    ...
                    <div x-show="$store.app.tab === 'keys'">
                        <div x-show="$store.app.selectedSlot == null" class="text-[11px] font-bold text-subtext1">
                        ...
                        <div x-show="$store.app.selectedSlot != null" x-cloak>
                            ...
                            <p class="text-[10px] font-bold text-subtext1 mb-3"
                               x-text="'now: ' + ($store.app.entryName($store.app.entryAt($store.app.selectedSlot || 0)) || 'nothing')"></p>

                            <input type="search" placeholder="Search keys…" class="pixel-input text-[11px] w-full mb-2"
                            ...
                            <div class="flex flex-wrap gap-2">
                                <button type="button" class="pixel-btn bg-surface1 px-3 py-1.5 text-[10px]" @click="$store.app.clearSlot()">Clear key</button>
                                <button type="button" class="pixel-btn bg-surface1 px-3 py-1.5 text-[10px]" @click="$store.app.resetSlotToFactory()">Factory default</button>
                            </div>
                        </div>
                    </div>
                    ...
                    <div x-show="$store.app.tab === 'lights'" x-cloak>
                        ...
                        <label class="block text-[10px] font-bold mb-1">Brightness <span x-text="$store.app.config?.brightness"></span>%</label>
                        <input type="range" min="0" max="100" class="w-full mb-3"
                               x-model.number="$store.app.config.brightness"
                               @change="$store.app.updateConfig({})">

                        <label class="block text-[10px] font-bold mb-1">Speed <span x-text="4 - ($store.app.config?.speed ?? 2)"></span>/4</label>
                        ...
                        <div x-show="($store.app.lightEffects.find(f => f.value === $store.app.config?.effect)?.direction || 0) > 0" x-cloak class="mb-3">
                            <label class="block text-[10px] font-bold mb-1">Direction</label>
                        ...
                        <div x-show="$store.app.lightEffects.find(f => f.value === $store.app.config?.effect)?.palette" x-cloak class="mb-3">
                            <label class="block text-[10px] font-bold mb-1">Base color</label>
                        ...
                        <p class="text-[10px] font-bold text-subtext1 mb-2">Switch the board to Paint mode and dab colors onto keys.</p>
                        <div class="flex flex-wrap gap-2">
                            <button type="button" class="pixel-btn bg-surface1 px-3 py-1.5 text-[10px]" @click="$store.app.paintAll()">Paint all keys</button>
                            <button type="button" class="pixel-btn bg-surface1 px-3 py-1.5 text-[10px]"
                        ...
                        <div class="grid grid-cols-2 gap-2 mb-2">
                            <label class="text-[10px] font-bold">Effect
                                <select class="pixel-input text-[10px] w-full mt-1"
                            ...
                            <label class="text-[10px] font-bold">Brightness
                                <input type="number" min="0" max="100" class="pixel-input text-[10px] w-full mt-1"
                    ...
                    <div x-show="$store.app.tab === 'feel'" x-cloak>
                        <div x-show="$store.app.selectedSlot == null" class="text-[11px] font-bold text-subtext1 mb-3">
                        ...
                        <template x-if="$store.app.selectedSlot != null">
                            <div>
                                ...
                                <template x-for="f in [
                                    ...
                                    <div class="mb-2">
                                        ...
                                        <template x-if="f[2] === 'select'">
                                            <select class="pixel-input text-[10px] w-full"
                                                    :value="$store.app.rt[$store.app.selectedSlot]?.[f[0]]"
                                ...
                                <div class="flex gap-2 flex-wrap">
                                    <button type="button" class="pixel-btn pixel-btn-green px-3 py-1.5 text-[10px]"
                    ...
                    <div x-show="$store.app.tab === 'macros'" x-cloak>
                        ...
                        <p class="text-[10px] font-bold text-subtext1 mb-2"
                        ...
                        <ol class="flex flex-col gap-1 mb-3 max-h-52 overflow-y-auto">
                            <template x-for="(a, i) in $store.app.macroSlots[$store.app.macroSlot]?.actions" :key="i">
                                <li class="flex items-center gap-1.5 bg-mantle border-2 border-crust px-1.5 py-1">
                                    <span class="pixel-chip bg-base text-[8px] font-pixel"
                        ...
                        <button type="button" class="pixel-btn pixel-btn-green px-3 py-1.5 text-[10px]" @click="$store.app.pushMacro()">Save macros</button>
                    ...
                    <div x-show="$store.app.tab === 'settings'" x-cloak>
                        ...
                        <div class="flex flex-col gap-2 mb-4">
                            ...
                            <label class="flex items-center gap-2 text-[10px] font-bold">Stability
                                <select class="pixel-input text-[10px]" :value="$store.app.config?.stability ?? 0"
                            ...
                            <label class="flex items-center gap-2 text-[10px] font-bold">Debounce
                                <select class="pixel-input text-[10px]" :value="$store.app.config?.debounce ?? 2"
                            ...
                            <label class="flex items-center gap-2 text-[10px] font-bold">Report rate
                                <select class="pixel-input text-[10px]" :value="$store.app.config?.reportRate ?? 1"
                        ...
                        <div class="mt-4 pt-3 border-t-2 border-surface1">
                            ...
                            <div class="flex flex-wrap gap-2">
                                <button type="button" class="pixel-btn bg-surface1 px-3 py-1.5 text-[10px]" @click="$store.app.resetBoard()">Restart board</button>
                                <button type="button" class="pixel-btn pixel-btn-red px-3 py-1.5 text-[10px]" @click="$store.app.factoryReset()">Factory reset</button>
                            </div>
                        </div>
                    </div>
                    ...
                    <div x-show="$store.app.tab === 'firmware'" x-cloak>
                        <div class="border-3 border-red bg-red/10 px-3 py-2 text-[10px] font-bold text-text mb-3">
                        ...
                        <p class="text-[10px] font-bold mb-3" x-text="'Current firmware: ' + ($store.app.fw || 'unknown')"></p>
                        <fieldset class="mb-3">
                            <legend class="text-[10px] font-bold mb-1">Firmware source</legend>
                            <label class="flex items-center gap-2 text-[10px] font-bold cursor-pointer">
                            ...
                            <input x-show="$store.app.fwSource === 'file'" type="file" accept=".bin"
                                   class="pixel-input text-[10px] w-full mt-1" @change="$store.app.pickFwFile($event)">
                            <p x-show="$store.app.fwSize" class="text-[10px] text-subtext0 font-bold mt-1" x-text="'Selected: ' + $store.app.fwSize + ' bytes'"></p>
                        </fieldset>
                        <button type="button" class="pixel-btn pixel-btn-red px-4 py-2 text-[11px]"
                        ...
                        <div x-show="$store.app.flashing || $store.app.flashProgress > 0" x-cloak class="mt-3">
                            ...
                            <p class="text-[10px] font-bold mt-1" x-text="$store.app.flashStage + ' — ' + $store.app.flashProgress + '%'"></p>
                        </div>
